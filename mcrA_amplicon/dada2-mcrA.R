...
#Identifying primers
FWD <- "GGTGGTGTMGGDTTCACMCARTA"  ## primer mcrA mlas 
REV <- "CGTTCATBGCGTAGTTVGGRTAGT"   ## primer mcrA-rev 
...
# (see mcra_testing.R) and now the parameters that I use is: maxEE (8,8), and truncLen = c(260,230)
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, truncLen = c(260,230),
...
mcra_fungene <- "mcrA-database-dada2.fasta"  # CHANGE ME to location on your machine
taxa_mcra <- assignTaxonomy(seqtab.nochim, mcra_fungene, multithread = TRUE, tryRC = TRUE)

#inspecting
taxa.print <- taxa_mcra  # Removing sequence rownames for display only
...
write.csv(taxa_mcra, file = "taxa_all_euk.csv")
