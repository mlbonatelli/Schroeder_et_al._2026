...
#Identifying primers
FWD <- "ACTCCTACGGGAGGCAGCAG"  ## primer 16S F (length 20)
REV <- "GGACTACHVGGGTWTCTAAT"   ## primer 16S R (length 20)

# primers used in DENG et al. 2017. Deng et al. 2018 does not have it, so I am using the same.
...
out <-  filterAndTrim(cutFs, filtFs, cutRs, filtRs, truncLen = c(270,230), #trimLeft = c(20, 20), # I did not find any primer sequence with the one they provided in the paper, that is why I am doing this here.
...
# Exporting tables
getwd()
write.csv(seqtab.nochim, file = "Deng_2_seqtab.nochim_16S.csv")
write.csv(taxa_16S, file = "Deng_2_taxa_16S.csv")
