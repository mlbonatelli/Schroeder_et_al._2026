...
get.sample.name <- function(fname) strsplit(basename(fname), "_")[[1]][1] #Change the last number to where your sample info is.
samples_sequenced <- unname(sapply(fnFs, get.sample.name))
samples_sequenced

#Identifying primers
FWD <- "CCTACGGGNGGCWGCAG"  ## primer 16S F 
REV <- "GACTACHVGGGTATCTAAKCC"   ## primer 16S R 
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% 401:428]
...
# Exporting tables
getwd()
write.csv(seqtab.nochim, file = "seqtab.nochim_16S.csv")
write.csv(taxa_16S, file = "taxa_16S.csv")
